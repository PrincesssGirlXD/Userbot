...
async def collect_afk_messages(UserBot: Client, message: Message):
    if AFK:
        ...
        if GetChatID(message) not in CHAT_TYPE:
            text = (
                f"`Konichiwa. I Am Anya Userbot.\n"
                f"My Master Is Not Available Right Now.\n"
                f"Last seen: {last_seen}\n"
                f"Reason: ```{AFK_REASON.upper()}```\n"
                f"When My Master Will Be Back He Will Reply You Thank You.`"
            )
            ...
            if CHAT_TYPE[GetChatID(message)] == 50:
                text = (
                    f"`Konichiwa. I Am Anya Userbot\n"
                    f"Last seen: {last_seen}\n"
                    f"This is the 10th time I've told you My Master Is AFK right now..\n"
                    f"He Will Reply You When He Will ComeBack.\n"
                ...
                text = (
                    f"`Hey My Master Flame is still not back yet.\n"
                    f"Last seen: {last_seen}\n"
